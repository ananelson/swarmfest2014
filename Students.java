...
public class Students extends SimState {
    ...
    public void initDataWriter() {
        try {
            /// "write-step-data-header"
            dataWriter = new BufferedWriter(new OutputStreamWriter(
                        new FileOutputStream("agitations.txt"), "utf-8"));
            dataWriter.write("agent\tperiod\tagitation\n");
            /// @end
        } catch (IOException ex) {
            System.out.println("Unable to open dataWriter." + ex);
        }
    }

    public void recordSimParams() {
        // Record params to JSON
        JSONObject obj=new JSONObject();
        obj.put("TEMPERING_CUT_DOWN", TEMPERING_CUT_DOWN);
        obj.put("TEMPERING_INITIAL_RANDOM_MULTIPLIER", TEMPERING_INITIAL_RANDOM_MULTIPLIER);
        obj.put("tempering", tempering);
        obj.put("numStudents", numStudents);
        obj.put("forceToSchoolMultiplier", forceToSchoolMultiplier);
        obj.put("randomMultiplier", randomMultiplier);

        Writer paramsWriter = null;

        try {
            paramsWriter = new BufferedWriter(new OutputStreamWriter(
                        new FileOutputStream("sim-params.json"), "utf-8"));
            paramsWriter.write(obj.toJSONString());
        } catch (IOException ex) {
            // report
        } finally {
            try {paramsWriter.close();} catch (Exception ex) {}
        }
    }

    public void finish() {
        Iterator yardIter = yard.iterator();
        try {dataWriter.close();} catch (Exception ex) {}
    }
